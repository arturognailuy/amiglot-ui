...
- **Profile minimums**:
  ...
  - Unique user handle (letters/numbers, case‑insensitive; stored **without** `@`, e.g., `arturo`; UI displays with `@` prefix; URLs use clean handle)
...
- **Profile edit rules**:
  ...
  - Internal fixed user ID is used for interactions; `@username` is just the display label generated by the UI
